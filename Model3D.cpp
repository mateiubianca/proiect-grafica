...
namespace gps {
	...
	GLuint Model3D::ReadTextureFromFile(const char* file_name) {
		...
		if (strcmp(file_name, "objects/grass/grass.png") == 0 || strcmp(file_name, "objects/vehicle/Vehicle.jpg") == 0)
			//|| strcmp(file_name, "objects/ground/2.jpg") == 0) 
		{
			format = GL_RGBA;
		}
		...
		glTexImage2D(
			GL_TEXTURE_2D,
			0,
			format, //GL_RGBA,//GL_SRGB, //GL_SRGB,//
			x,
			y,
			0,
			GL_RGBA,
